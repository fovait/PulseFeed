...
func (w *NotificationWorker) process(ctx context.Context, d amqp.Delivery) error {
	...
	case routingKey == "like.like":
		...
		authorID, err := w.getVideoAuthor(ctx, evt.VideoID)
		if err != nil {
			return err
		}
		if authorID == 0 || authorID == evt.AccountID {
			return nil
		}
		notif = &Notification{RecipientID: authorID, SenderID: evt.AccountID, Type: "like", TargetID: evt.VideoID, Content: "点赞了你的视频"}
...
func (w *NotificationWorker) getVideoAuthor(ctx context.Context, videoID uint) (uint, error) {
	var authorID uint
	err := w.db.WithContext(ctx).Table("videos").
		Where("id = ?", videoID).
		Select("author_id").
		Scan(&authorID).Error
	return authorID, err
