...
func (w *PopularityWorker) process(ctx context.Context, body []byte) error {
	...
	if err := json.Unmarshal(body, &evt); err != nil {
		log.Printf("popularity worker: invalid json: %v", err)
		return nil
	}
	if evt.VideoID == 0 || evt.Change == 0 {
		log.Printf("popularity worker: invalid event: %+v", evt)
		return nil
	}
	return video.UpdatePopularityCache(ctx, w.cache, evt.VideoID, evt.Change)
}
